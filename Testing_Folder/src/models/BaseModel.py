...
class BaseModel(pl.LightningModule, ABC):
    ...
    def on_test_epoch_end(self) -> None:
        """Log the test PR curve and confusion matrix after predicting all test samples."""
        conf_mat = self.conf_mat.compute().cpu().numpy()
        
        # Comment out WandB logging for confusion matrix
        ...
        # else:
        print("WandB is disabled; skipping logging for Test confusion matrix.")

        fig, ax = self.test_pr_curve.plot(score=True)

        # Comment out WandB logging for PR curve
        # if wandb.run is not None:  # Check again for WandB initialization
        #     wandb.log({"Test PR Curve": wandb.Image(fig)})
        # else:
        print("WandB is disabled; skipping logging for Test PR Curve.")
